...
*Last updated: 2026-07-18 · Status: **draft** (good-shape internal report for later polish, not camera-ready).*
...
**(f) The DE wall is format-independent — external knowledge as a live LLM agent does not crack it either.** A final pre-deadline sprint fired the strongest remaining DE attack: a Track B **per-row LLM agent** (gpt-oss-120b) reasoning over *retrieved external biology* — each pair's GO:BP terms and STRING partners pulled live from mygene.info / string-db — with the abstention collapse fixed (0% `0/0`). Because the agent sees knowledge the leakage-allowed oracle (§5a) never had, beating 0.555 was possible in principle. It did not: a single 150-row dual-OOD read teased **AUROC_de = 0.631**, but the trustworthy **1,500-row (3 × 500-seed)** read regressed to **AUROC_de = 0.578, 95% CI [0.549, 0.607]** — the CI *includes* the 0.555 ceiling, and the fused (agent-DE ⊕ neighbour-DIR) candidate scored **0.604 ± 0.035** vs the 0.597 incumbent, i.e. statistical parity, not a gain. The pipeline was verified **leak-free** (no label-fitted component; dual-OOD `assert_leak_free`), so the null is honest. The same sprint also ruled out a **DepMap co-essentiality** direction key (standalone DIR-AUROC 0.547 « STRING 0.651; fusing it *lowers* the score to 0.637), confirming the ~0.65 DIR ceiling from a fresh key. The lesson repeats the §3.1 discipline: the 0.631 was small-sample noise (per-seed DE swings ±0.04, straddling chance), and would have been a false breakthrough had we submitted on it. So DE-vs-none now resists **every** attack format — curated edges, learned models, retrieval lookup, chain-of-thought, and agentic external-knowledge retrieval — which **strengthens the DE-unlearnable thesis of §5a rather than qualifying it** [`progress-report.md` "Leaderboard sprint (2026-07-18)"; `findings/retrieval-agent-de-headline-was-noise.md`; `findings/coessentiality-direction-key-negative.md`; `findings/external-knowledge-does-not-crack-de.md`]. No Kaggle quota was spent; the honest ceiling (A 0.586 / B 0.597) stands and was banked.
...
- `knowledge/wiki/findings/retrieval-agent-de-headline-was-noise.md` — the Track B external-knowledge agent's AUROC_de=0.631 was 150-row noise; the trustworthy 1,500-row read is 0.578 [0.549, 0.607] ≈ parity; leak-free.
- `knowledge/wiki/findings/coessentiality-direction-key-negative.md` — DepMap co-essentiality is a weaker direction key (0.547) than STRING (0.651) and drags the fusion down; the ~0.65 DIR ceiling holds.
- `knowledge/wiki/findings/external-knowledge-does-not-crack-de.md` — sprint synthesis: external biology as live LLM-agent retrieval lands at DE parity, not above; the DE wall is format-independent.
