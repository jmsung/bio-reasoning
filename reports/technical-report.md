...
4. **DE is done — closed, not deferred.** Every remaining DE angle has now been fired and missed (§5a): the marginal-property shot, native-mouse cross-dataset transfer (real-LB **Δ−0.005**), a structural audit (82/96 test perts are essential genes never screened in any borrowable dataset), and gpt-oss chain-of-thought DE calls (≈ chance, synthpert 18/51). An error-analysis dissection of the incumbent adds the final constraint: DE-AUROC is at chance across *every* perturbation category (not a concealed sub-population), with neighbour coverage already at 98.9%, so no subset-gating or graph-expansion shot remains [`track-a-error-structure.md`]. No further DE shots are planned.
