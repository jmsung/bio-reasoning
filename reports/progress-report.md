...
| **Track B — neighbour-DIR fusion (v5)** | fuse the same neighbour-retrieval direction into the floored Track B submission via `fuse_neighbour_direction` — the #28 lever, different base | **OOD-val 0.5916** (LB pending) | **new Track B best offline** (+0.020 over the 0.578 DIR-blend); direction 0.570→0.624 at 98% coverage, DE unchanged (0.559→0.560) — the 0.578 blend had *not* already captured the neighbour direction |
...
**Kaggle LB still pending** (user/Bing-gated upload); the OOD-val↔LB gap has held to ~0.004
across prior submissions, so the board should confirm. Pure feature channel, no LLM/Bing
dependency. Schema-valid submission built and verified (1,813 rows, 99.6% coverage, 0 nulls),
archived at `mb/findings/solutions/track-b-de-dir-OODval0.5916.csv`.
