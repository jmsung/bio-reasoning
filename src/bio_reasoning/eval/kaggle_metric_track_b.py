"""Track B Kaggle metric copied from the official challenge baseline."""
...
def score(solution: pd.DataFrame, submission: pd.DataFrame, row_id_column_name: str) -> float:
    ...
    if missing:
        raise ParticipantVisibleError(
            ...
            "See the sample submission file for the expected format."
        )
    ...
    if max_tc > MAX_TOOL_CALLS_PER_ROW:
        ...
        raise ParticipantVisibleError(
            ...
            "Reduce the number of tool calls per question."
        )
    ...
    if max_pt > MAX_PROMPT_TOKENS:
        raise ParticipantVisibleError(
            ...
            f"but submission reports {int(max_pt):,}. Shorten your system prompt or tool definitions."
        )
    ...
    if max_dt > MAX_DISTINCT_TOOLS:
        raise ParticipantVisibleError(
            ...
            f"but submission reports {int(max_dt)}. Reduce the number of tool definitions."
        )

    for col in ("prediction_up", "prediction_down"):
        if col not in merged.columns:
            raise ParticipantVisibleError(f"Submission is missing required column '{col}'.")
